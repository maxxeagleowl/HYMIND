...
class TestEmbeddingsMissingApiKey:
    ...
    def test_embedding_model_defaults_to_small(self):
        from rag.embeddings import get_embedding_model, DEFAULT_EMBEDDING_MODEL
...
class TestFindingToMetadata:
    def test_all_required_keys_present(self):
        from rag.pinecone_store import finding_to_metadata
    ...
    def test_snippet_truncated_to_limit(self):
        from rag.pinecone_store import finding_to_metadata, _SNIPPET_LIMIT
...
class TestMakeVectorId:
    def test_same_url_produces_same_id(self):
        from rag.pinecone_store import make_vector_id
...
class TestUpsertEmptyFindings:
    def test_empty_list_returns_zero(self):
        from rag.pinecone_store import upsert_findings
...
class TestUpsertCallsMock:
    ...
    def test_upsert_vector_has_id_values_metadata(self):
        from rag.pinecone_store import upsert_findings, make_vector_id
...
class TestWorkflowWithRagDisabled:
    def test_build_workflow_succeeds(self):
        from workflows.research_workflow import build_workflow
        app = build_workflow()
...
class TestFindingsFromState:
    def test_merged_results_converted_to_stored_findings(self):
        from rag.retriever import findings_from_state
        state = {
            "merged_results": [
...
class TestRagContextSerializationInWorkflow:
    def test_retrieve_node_returns_dicts_not_dataclasses(self):
        ...
        with patch("rag.retriever.is_pinecone_configured", return_value=True), \
             patch("rag.retriever.create_embeddings", return_value=[_fake_embedding()]), \
             patch("rag.retriever.query_index", return_value=[{
                 "id": "abc", "score": 0.9,
                 "metadata": {
    ...
    def test_rag_context_included_in_report_build_context(self):
        ...
        from reporting.report_generator import build_context
        from workflows.state import initial_state
